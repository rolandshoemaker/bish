#include <cassert>
#include "CodeGen_Bourne.h"

using namespace Bish;

void CodeGen_Bourne::indent() {
    for (unsigned i = 0; i < indent_level; i++) {
        stream << "    ";
    }
}

void CodeGen_Bourne::visit(const Module *n) {
...
void CodeGen_Bourne::visit(const Block *n) {
...
void CodeGen_Bourne::visit(const Variable *n) {
    if (should_quote_variable()) stream << "\"";
    stream << "$" << lookup_name(n);
    if (should_quote_variable()) stream << "\"";
}

void CodeGen_Bourne::visit(const ReturnStatement *n) {
    stream << "echo ";
...
void CodeGen_Bourne::visit(const IfStatement *n) {
    stream << "if [[ ";
    enable_functioncall_wrap();
...
void CodeGen_Bourne::visit(const ForLoop *n) {
...
void CodeGen_Bourne::visit(const Function *n) {
    stream << "function bish_" << n->name << " ";
    stream << "() ";
    LetScope *s = new LetScope();
    push_let_scope(s);
    // Bourne doesn't allow named arguments to functions.
...
void CodeGen_Bourne::visit(const FunctionCall *n) {
...
void CodeGen_Bourne::visit(const ExternCall *n) {
    if (should_functioncall_wrap()) stream << "$(";
    disable_quote_variable();
...
void CodeGen_Bourne::visit(const Assignment *n) {
...
void CodeGen_Bourne::visit(const BinOp *n) {
...
void CodeGen_Bourne::visit(const UnaryOp *n) {
    switch (n->op) {
    case UnaryOp::Negate:
        stream << "-";
        break;
...
void CodeGen_Bourne::visit(const Integer *n) {
    stream << n->value;
}

void CodeGen_Bourne::visit(const Fractional *n) {
    stream << n->value;
}

void CodeGen_Bourne::visit(const String *n) {
    stream << "\"" << n->value << "\"";
}

void CodeGen_Bourne::visit(const Boolean *n) {
    stream << n->value;
}
